...
function renderTable() {
  ...
  const filtered = complaints.filter(c => {
    const matchStatus = statusValue === "all" || c.status.toLowerCase() === statusValue;
    const matchCategory = categoryValue === "all" || c.category === categoryValue;
  ...
  if (isAdmin) {
    document.querySelectorAll(".statusSelect").forEach(select => {
      select.addEventListener("change", function () {
        ...
        const realIndex = allComplaints.findIndex(c =>
